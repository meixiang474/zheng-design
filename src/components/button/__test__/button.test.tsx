...
describe('test Button component', () => {
  it('should render the correct default button', () => {
    ...
    expect(ele).not.toHaveAttribute('inLink');
    // 正常添加classname
    expect(
      ele
        .getAttribute('class')
    ...
    // span正常显示
    expect(ele.getElementsByTagName('span')[0]).toBeInTheDocument();
    // 正常默认属性
  ...
  it('should render loading', () => {
    let wrapper = render(<Button isLoading>hello</Button>);
    expect(wrapper).toMatchSnapshot();
    const ele = wrapper.getByTestId('button');
    expect(ele).toBeInTheDocument();
    ...
    const text2 = wrapper.getByText('yehuozhili');
    expect(text2).toBeInTheDocument();
  });
  it('should render correct with isUnclickable', () => {
    const wrapper = render(
      <Button isUnclickable {...disabledProps}>
        hello
      </Button>
    );
    expect(wrapper).toMatchSnapshot();
